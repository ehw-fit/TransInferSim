...
class ConvolutionalModel(ABC):
    """Abstract class for a convolutional model for hardware inference analysis.

    Args:
        name (str): The name of the model.
        input_shape (tuple): The shape of the input (channels, height, width).
        batch_size (int): The batch size for the model.
        add_bias (bool): Whether to account for bias parameters.
        im2col (bool, optional): Whether to use im2col optimization. Defaults to True.
    """
    def __init__(self, name: str, input_shape: tuple, batch_size: int, add_bias: bool, im2col: bool = True, **kwargs):
        assert len(input_shape) == 3, "Input shape must be a tuple of (channels, height, width)"
        assert all(dim > 0 for dim in input_shape), "All dimensions in input shape must be positive"
    ...
    def add_layer(self, layer: Layer):
        """
        Adds a layer to the convolutional model, ensuring that the layer name is unique.
    ...
    def total_parameters(self):
        """Calculates the total number of parameters in the convolutional model."""
        return self.num_static_parameters

    def total_computations(self):
        """Calculates the total number of computations for the convolutional model."""
