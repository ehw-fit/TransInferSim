...
class OffChipMemory(GenericMemory):
    def __init__(self, name: str, width: int, depth: int, action_latency: float, bus_bitwidth: int, cycle_time: float, word_size: int = 8, ports: int = 2, banks: int = 1, replacement_strategy = None, parent_component = None):
        super().__init__(width=width, depth=depth, bus_bitwidth=bus_bitwidth, action_latency=action_latency, word_size=word_size, ports=ports, name=name, cycle_time=cycle_time, replacement_strategy=replacement_strategy, parent_component=parent_component, accelergy_class="offchip_DRAM")
    ...
    def read(self, read_mode, data_id, tensor_shape, data_bitwidth, port_id, tile_shape=None, elems_to_read=None, offset=None):
        ...
        if read_mode == "read_tile":
            ...
            if not tile_found:
                stats = self.parent_component.get_statistics(log_mem_contents=True)
                self.parent_component.pretty_print_stats(stats, verbose=False, file_path="debug_stats3.txt")
        ...
        mem_reads = int(math.ceil(read_mem_words / self.block_size))
    ...
    def write(self, write_mode, data_id, tensor_shape, data_bitwidth, port_id, tile_shape=None, elems_to_write=None, offset=None, verbose=False, **kwargs):
        """
    ...
    def get_accelergy_description(self):
        ...
        return {
            ...
            "attributes": {
                "depth": self.depth,
                "width": self.width,
                "block_size": self.block_size,  # Number of words per row as per Accelergy
                "cluster_size": 1,
                "n_banks": self.banks,
                "datawidth": self.word_size,
                "read_bandwidth": self.bus_bitwidth,
                "write_bandwidth": self.bus_bitwidth,
