...
class SharedMemory(GenericMemory):
    def __init__(self, name: str, width: int, depth: int, action_latency: float, bus_bitwidth: int, cycle_time: float, word_size: int, power_gating: bool = False, banks: int = 1, ports: int = 2, replacement_strategy="random", parent_component = None):
        super().__init__(width=width, depth=depth, bus_bitwidth=bus_bitwidth, action_latency=action_latency, cycle_time=cycle_time, word_size=word_size, banks=banks, ports=ports, name=name, replacement_strategy=replacement_strategy, parent_component=parent_component, accelergy_class="smartbuffer_SRAM")
