...
class ViTSmall(TransformerModel):
    def __init__(self, name: str = "vit_small", image_size: int = 224, patch_size: int = 16, num_layers: int = 12, embedding_dim: int = 384, ffn_layer_dim: int = 1536, num_heads: int = 6, batch_size: int = 1, add_bias: bool = False, **kwargs):
    ...
    def define_parameters(self):
        """Aggregate parameters from all layers to define the parameters for the ViT-Small model."""
        static_params = {}
        dynamic_params = {}
    ...
    def define_plan(self):
        """Aggregate execution plans from all layers to define the execution plan for the ViT-Small model."""
        operations = []
        for i, layer in enumerate(self.layers):
